...
class DemucsStreamer:
    ...
    def feed(self, wav):
        ...
        while self.input_pointer <= self.how_many_samples_input:
            self.input_counter[self.input_pointer] = begin
...
def stream_test():
    ...
    parser = argparse.ArgumentParser(
        "body.model.demucs",
        description="Benchmark the streaming Demucs implementation. "
        "In this test, we won't really care about the delta(as it is seen to be <<<<1%); "
        "instead we will look at the average latency.",
    )
    # model architecture is pre-determined and set
    # parser.add_argument("--depth", default=5, type=int) -> 5
    parser.add_argument("--resample", default=4, type=int)
    # parser.add_argument("--hidden", default=48, type=int) -> 64
    # parser.add_argument("--sample_rate", default=16000, type=float) -> 16000
    # parser.add_argument("--device", default="cpu") -> cpu
    parser.add_argument("-t", "--num_threads", type=int)
    parser.add_argument("-f", "--num_frames", type=int, default=4)
    ...
    net = Demucs(hidden=64, resample=resample).to("cpu")

    print("stream test degins")
    input_length = 16000 * benchmark_length
    time_per_step = latency_interval / 16000
    recorded_samples = th.rand(1, input_length, device="cpu", pin_memory=True)
    streamer = DemucsStreamer(
        net, num_frames=num_frames, latency_interval=latency_interval
    )
    ...
    print(f"mean time-per-frame: {streamer.time_per_frame: 2f}")
...
def mac_test():
    ...
    model = Demucs(hidden=64, resample=4).to("cpu")
    siglen = 2**17
    input = th.randn(1, siglen)
