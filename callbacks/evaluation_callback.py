...
def evaluation_callback(
    ...
    if step == 0:
        sf.write(
            paths["degraded"] + "y.wav",
            y_train[0].detach().cpu().numpy(),
            args.exp.sample_rate,
        )
        sf.write(
            paths["original"] + "x.wav",
            reference_train[0].detach().cpu().numpy(),
            args.exp.sample_rate,
        )

    sf.write(
        paths["reconstructed"] + "x_hat" + step + ".wav",
        y_hats_tweedie[0].detach().cpu().numpy(),
        args.exp.sample_rate,
    )
    ...
    x_test_batch = torch.stack(x_test)
    print("x_test_batch", x_test_batch.shape)
    y_hats_test_batch = operator.degradation(x_test_batch.squeeze(1))

    for i in range(len(x_test)):
        y_hats_test.append(y_hats_test_batch[i])
    ...
    try:
        for m in metrics:
            if m == "ESR":
                ESR = tt_utils.compute_ESR(y_test_test, y_hats_test)
                print(f"ESR_FWR_test: {ESR}")
                wandb_run.log({f"ESR_FWR_test": ESR}, step=step)
            ...
            if m == "STFT_mag":
                STFT_mag = tt_utils.compute_STFT_mag(
                    y_test_test, y_hats_test, fft_size=1024, hop_size=256
                )
                print(f"STFT_mag_test: {STFT_mag}")
                wandb_run.log({f"STFT_mag_test": STFT_mag}, step=step)
            ...
            if m == "LSD":
                ...
                print(f"LSD_test: {STFT_mag}")
                wandb_run.log({f"LSD_test": STFT_mag}, step=step)
            ...
            if m == "freq_error":
                ...
                wandb_run.log({"Freq_error_test": fig}, step=step)
        ...
        wandb_run.log(
            {
                "y_test": [
                    wandb.Audio(
                        torch.stack(y_test_test).view(-1).cpu(),
                        caption="y_test",
        ...
        wandb_run.log(
            {
                "x_test": [
                    wandb.Audio(
                        torch.stack(x_test_test).view(-1).cpu(),
                        caption="x_test",
                        sample_rate=args.exp.sample_rate,
                    )
                ]
        ...
        wandb_run.log(
            {
                "y_hats_test": [
                    wandb.Audio(
                        torch.stack(y_hats_test).view(-1).cpu(),
                        caption="y_hats_test",
                        sample_rate=args.exp.sample_rate,
                    )
                ]
    ...
    try:
        ...
        print("response", response.shape, responsepred.shape, ramp.shape)
        fig.update_yaxes(range=[-1, 1])
    except:
        print("response not computed")
    else:
    ...
    try:
        fig = go.Figure()
        coefs = operator.params["spline_coefs"].detach().cpu()
        fig.add_trace(
            go.Scatter(
                np.arange(coefs.shape[-1]), coefs, mode="lines", name="spline_coefs"
            )
        )
    ...
    wandb_run.log(
        {
            "x_tweedie": wandb.Audio(
                x_den[0:8].view(-1).detach().cpu().numpy() / maxim,
                sample_rate=args.exp.sample_rate,
            )
        },
        step=step,
