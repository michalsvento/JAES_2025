...
def do_stft(noisy, clean=None, win_size=2048, hop_size=512, device="cpu", DC=True):
    ...
    noisy = torch.cat(
        (noisy, torch.zeros(noisy.shape[0], win_size).to(noisy.device)), 1
    )
    stft_signal_noisy = torch.stft(
        noisy,
        win_size,
        hop_length=hop_size,
        window=window,
...
def plot_norms(path, normsscores, normsguides, t, name):
    ...
    df = pd.DataFrame.from_dict(
        {
            "sigma": values[0:-1],
            "score": normsscores.cpu().numpy(),
            "guidance": normsguides.cpu().numpy(),
        }
    )
    fig = px.line(
        df, x="sigma", y=["score", "guidance"], log_x=True, log_y=True, markers=True
    )
...
def plot_loss_by_sigma_and_freq(
    sigma_freq_means, sigma_freq_stds, sigma_bins, freq_bins
):
    df = pd.DataFrame.from_dict({"sigma": np.tile(sigma_bins, len(freq_bins))})
    names = []
    means = []
    stds = []
...
def plot_melspectrogram(X, refr=1):
    ...
    for i in range(
        X.shape[0]
    ):  # iterate over batch size, shity way of ploting all the batched spectrograms
        o = S_db[i]
        if i == 0:
            res = o
        else:
...
def plot_cpxCQT_from_raw_audio(x, args, refr=None):
    ...
    device = x.device
    CQTransform = CQT_cpx(
        fmin, fbins, args.sample_rate, args.audio_len, device=device, split_0_nyq=False
    )
...
def diffusion_CQT_animation(
    path, x, t, args, refr=1, name="animation_diffusion", resample_factor=1
):
    """
    ...
    fig = px.imshow(
        allX, animation_frame=0, zmin=-40, zmax=10, binary_compression_level=0
    )  # I need to incorporate t here!!!
...
def diffusion_joint_filter_animation(
    path, score, grads, f, t, refr=1, name="animation_diffusion", NT=15
):
    """
    ...
    fig = px.line(
        df, x="f", y=["score", "grads"], animation_frame="sigma", log_x=True
    )  # I need to incorporate t here!!!
...
def diffusion_filter_animation(
    path, x, f, t, refr=1, name="animation_diffusion", NT=15
):
    """
    plot an animation of the reverse diffusion process of filters
    args:
    ...
    x = x.reshape(-1)
    df = pd.DataFrame(
        {"f": f.cpu().numpy(), "x": x.cpu().numpy(), "sigma": sigma.cpu().numpy()}
    )
    fig = px.line(
        df, x="f", y="x", animation_frame="sigma", log_x=True
    )  # I need to incorporate t here!!!
    # t=t[i_s].cpu().numpy()
    # t=t.cpu().numpy()
...
def diffusion_spec_animation(
    path, x, t, stft, refr=1, name="animation_diffusion", NT=15
):
    """
    plot an animation of the reverse diffusion process of filters
    args:
    ...
    allX = allX.cpu().numpy()
    fig = px.imshow(
        allX, animation_frame=0, zmin=-40, zmax=20
    )  # I need to incorporate t here!!!
...
def plot_rir(rir, T_separation, fs, path=None, name=None, save=True):
    ...
    RIR = torch.stft(
        ...
        n_fft=512,
        hop_length=128,
        window=torch.hann_window(512).to(rir.device),
        return_complex=True,
    )
    ...
    ax[1].plot(torch.linspace(0, rir.size(-1) / fs, rir.size(-1)), rir)
