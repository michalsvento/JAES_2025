...
def whitening(x, state_dict):
    n_fft = state_dict["n_fft"]
    window = state_dict["window"]

    window = window.to(x.device)

    hop_length = state_dict["hop_length"]
    win_length = state_dict["win_length"]
    ...
    if X.ndim == 2:
        X_whiten = X * torch.sqrt(state_dict["whitener"]).unsqueeze(-1).to(x.device)
    elif X.ndim == 3:
        # batched case
        X_whiten = X * torch.sqrt(state_dict["whitener"]).unsqueeze(0).unsqueeze(-1).to(
            x.device
        )
    else:
...
def dewhitening(x, state_dict):
    n_fft = state_dict["n_fft"]
    window = state_dict["window"].to(x.device)
    hop_length = state_dict["hop_length"]
    win_length = state_dict["win_length"]
    sample_rate = state_dict["sample_rate"]
    ...
    if X.ndim == 2:
        X_whiten = X * torch.sqrt(state_dict["dewhitener"]).unsqueeze(-1).to(x.device)
    elif X.ndim == 3:
        # batched case
        X_whiten = X * torch.sqrt(state_dict["dewhitener"]).unsqueeze(0).unsqueeze(
            -1
        ).to(x.device)
    else:
        raise Exception("X has wrong shape")
