...
class CQT_nsgt:
    def __init__(
        ...
        if mode == "flex_oct":
            self.scale = FlexLogOctScale(
                fs, self.numocts, self.binsoct, time_reductions
            )
        else:
        ...
        self.ncoefs = max(
            int(math.ceil(float(len(gii)) / mii)) * mii
        ...
        self.gd = nsdual(
            self.g, self.wins, self.nn, self.M, dtype=self.dtype, device=self.device
        )

        # filter DC
        self.Hlpf = torch.zeros(self.Ls, dtype=self.dtype, device=self.device)
        self.Hlpf[0 : len(self.g[0]) // 2] = (
            self.g[0][: len(self.g[0]) // 2]
            * self.gd[0][: len(self.g[0]) // 2]
            * self.M[0]
        )
        self.Hlpf[-len(self.g[0]) // 2 :] = (
            self.g[0][len(self.g[0]) // 2 :]
            * self.gd[0][len(self.g[0]) // 2 :]
            * self.M[0]
        )
        ...
        self.Hlpf[self.wins[nyquist_idx][0 : (Lg + 1) // 2]] += (
            self.g[nyquist_idx][(Lg) // 2 :]
            * self.gd[nyquist_idx][(Lg) // 2 :]
            * self.M[nyquist_idx]
        )
        self.Hlpf[self.wins[nyquist_idx][-(Lg - 1) // 2 :]] += (
            self.g[nyquist_idx][: (Lg) // 2]
            * self.gd[nyquist_idx][: (Lg) // 2]
            * self.M[nyquist_idx]
        )
        ...
        if mode == "matrix" or mode == "oct" or mode == "matrix_pow2":
            sl = slice(
                1, len(self.g) // 2
            )  # getting rid of the DC component and the Nyquist
        else:
            sl = slice(0, len(self.g) // 2 + 1)
        ...
        def get_ragged_giis(g, wins, ms, mode):
            ...
            # ragged_giis=[]
            c = torch.zeros(
                (len(g), self.Ls // 2 + 1), dtype=self.dtype, device=self.device
            )
            ix = []
            if mode == "oct":
                for i in range(self.numocts):
                    ix.append(
                        torch.zeros(
                            (self.binsoct, self.size_per_oct[i]),
                            dtype=torch.int64,
                            device=self.device,
                        )
                    )
                ...
                ix.append(
                    torch.zeros((1, ms[0]), dtype=torch.int64, device=self.device)
                )
                count = 0
                ...
                ix.append(
                    torch.zeros(
                        (count, ms[i - 1]), dtype=torch.int64, device=self.device
                    )
                )

                ix.append(
                    torch.zeros((1, ms[-1]), dtype=torch.int64, device=self.device)
                )
            ...
            for i, (gii, win_range) in enumerate(zip(g, wins)):
                if i > 0:
                    if ms[i] != ms[i - 1] or (
                        (mode == "oct_complete" or mode == "matrix_complete")
                        and (j == 0 or i == len(g) - 1)
                    ):
                        j += 1
                        k = 0
                ...
                if (i == 0 or i == len(g) - 1) and (
                    ...
                    if i == 0:
                        ...
                        ix[j][0, -(Lg // 2) :] = torch.flip(
                            win_range[Lg // 2 :].unsqueeze(0), (-1,)
                        )
                    if i == len(g) - 1:
                        c[i, win_range[: (Lg + 1) // 2]] = gii[..., : (Lg + 1) // 2]

                        ix[j][0, : (Lg + 1) // 2] = torch.flip(
                            win_range[: (Lg + 1) // 2].unsqueeze(0), (-1,)
                        )  # rethink this
        ...
        if (
            self.mode == "matrix"
            or self.mode == "matrix_complete"
            or self.mode == "matrix_pow2"
        ):
            self.giis, self.idx_enc = get_ragged_giis(
                self.g[sl], self.wins[sl], self.M[sl], self.mode
            )
            ...
            ragged_giis = [
                torch.nn.functional.pad(
                    torch.unsqueeze(gii, dim=0), (0, self.maxLg_enc - gii.shape[0])
                )
        ...
        def get_ragged_gdiis(gd, wins, mode, ms=None):
            ...
            ix = (
                torch.zeros(
                    (len(gd), self.Ls // 2 + 1), dtype=torch.int64, device=self.device
                )
                + self.maxLg_dec // 2
            )  # I initialize the index with the center to make sure that it points to a 0
            for i, (g, win_range) in enumerate(zip(gd, wins)):
                Lg = g.shape[0]
                gl = g[: (Lg + 1) // 2]
                gr = g[(Lg + 1) // 2 :]
                zeros = torch.zeros(
                    self.maxLg_dec - Lg, dtype=g.dtype, device=g.device
                )  # pre-allocation
                ...
                if mode == "matrix_complete" and i == 0:
                    ...
                    ix[i, wr2] = (
                        torch.Tensor([i for i in range(len(wr2))])
                        .to(torch.int64)
                        .to(self.device)
                    )  # the start part
                    ...
                    ix[i, wr1] = (
                        torch.Tensor(
                            [self.maxLg_dec - (Lg // 2) + i for i in range(len(wr1))]
                        )
                        .to(torch.int64)
                        .to(self.device)
                    )  # the end part
            ...
            return (
                torch.conj(torch.cat(ragged_gdiis)).to(self.dtype) * self.maxLg_dec,
                ix,
            )
        ...
        def get_ragged_gdiis_critical(gd, ms):
            ...
            for i, (g, m) in enumerate(zip(gd, ms)):
                ...
                gr = g[(Lg + 1) // 2 :]
                zeros = torch.zeros(
                    m - Lg, dtype=g.dtype, device=g.device
                )  # pre-allocation
        ...
        def get_ragged_gdiis_oct(gd, ms, wins, mode):
            ...
            if mode == "oct_complete":
                ix += [
                    torch.zeros(
                        (1, self.Ls // 2 + 1), dtype=torch.int64, device=self.device
                    )
                    + ms[0] // 2
                ]
            ...
            ix += [
                torch.zeros(
                    (self.binsoct, self.Ls // 2 + 1),
                    dtype=torch.int64,
                    device=self.device,
                )
                + self.size_per_oct[j] // 2
                for j in range(len(self.size_per_oct))
            ]
            if mode == "oct_complete":
                ix += [
                    torch.zeros(
            ...
            for i, (g, m, win_range) in enumerate(zip(gd, ms, wins)):
                if (
                    i > 0
                    and m != mprev
                    or (mode == "oct_complete" and i == len(gd) - 1)
                ):
                ...
                gr = g[(Lg + 1) // 2 :]
                zeros = torch.zeros(
                    int(m - Lg), dtype=g.dtype, device=g.device
                )  # pre-allocation
                paddedg = torch.cat((gl, zeros, gr), 0).unsqueeze(0) * m
                ragged_gdiis.append(paddedg)
                mprev = m
                ...
                if mode == "oct_complete" and i == 0:
                    ...
                    # ix[i,wr1]=torch.Tensor([self.maxLg_dec-(Lg//2)+i for i in range(len(wr1))]).to(torch.int64) #the end part
                    ix[-1][k, wr1] = (
                        torch.Tensor([m - (Lg // 2) + i for i in range(len(wr1))])
                        .to(self.device)
                        .to(torch.int64)
                    )  # the end part
                    # ix[i,wr2]=torch.Tensor([i for i in range(len(wr2))]).to(torch.int64) #the start part
        ...
        if self.mode == "matrix" or self.mode == "matrix_complete":
            self.gdiis, self.idx_dec = get_ragged_gdiis(
                self.gd[sl], self.wins[sl], self.mode
            )
            # self.gdiis = self.gdiis[sl]
            # self.gdiis = self.gdiis[0:(self.gdiis.shape[0]//2 +1)]
        elif self.mode == "matrix_pow2":
            self.gdiis, self.idx_dec = get_ragged_gdiis(
                self.gd[sl], self.wins[sl], self.mode, ms=self.M[sl]
            )
        elif self.mode == "oct" or self.mode == "oct_complete":
            self.gdiis, self.idx_dec = get_ragged_gdiis_oct(
                self.gd[sl], self.M[sl], self.wins[sl], self.mode
            )
            ...
            ragged_gdiis = [
                torch.nn.functional.pad(
                    torch.unsqueeze(gdii, dim=0), (0, self.maxLg_dec - gdii.shape[0])
                )
                for gdii in self.gd
            ]
            self.gdiis = torch.conj(torch.cat(ragged_gdiis))
    ...
    def apply_hpf_DC(self, x):
        ...
        if Lin < self.Ls:
            ...
            raise ValueError(
                "Input signal is longer than the maximum length. I could have patched it, but I didn't. sorry :("
            )
    ...
    def nsgtf(self, f):
        ...
        if self.mode == "matrix" or self.mode == "matrix_pow2":
            ...
            t = (
                ft.unsqueeze(-2) * self.giis
            )  # this has a lot of rendundant operations and, probably, consumes a lot of memory. Anyways, it is parallelizable, so it is not a big deal, I guess.
            ...
            a = torch.gather(
                ...
                .unsqueeze(0)
                .unsqueeze(0)
                .expand(t.shape[0], t.shape[1], -1, -1),
            )  # To make torch.gather broadcast, I need to add a dimension to the index.
            ...
            for i in range(self.numocts):
                ...
                a = torch.gather(
                    t[..., i * self.binsoct : (i + 1) * self.binsoct, :],
                    3,
                    self.idx_enc[i]
                    .unsqueeze(0)
                    .unsqueeze(0)
            ...
            for i in range(self.numocts):
                ...
                # ret.append(torch.fft.ifft(torch.cat(bucketed_tensors,2)))
                a = torch.gather(
                    t[..., i * self.binsoct + 1 : (i + 1) * self.binsoct + 1, :],
                    3,
                    self.idx_enc[i + 1]
                    .unsqueeze(0)
                    .unsqueeze(0)
            ...
            a[..., : (L) // 2] = torch.conj(
                a[..., : (L) // 2]
            )  # conjugate one of the parts (here the first)
            ret.append(torch.fft.ifft(a))
            ...
            for j, (mii, win_range, Lg, col) in enumerate(self.loopparams_enc):
                ...
                c[:, :, j, sl2] = t[
                    :, :, : Lg // 2
                ]  # if mii is odd, this is of length mii//2
            ...
            for j, (mii, win_range, Lg, col) in enumerate(self.loopparams_enc):
                ...
                t = ft[:, :, win_range] * torch.fft.fftshift(
                    self.giis[j, :Lg]
    ...
    def nsigtf(self, cseq):
        ...
        if (
            self.mode != "matrix"
            and self.mode != "matrix_slow"
            and self.mode != "matrix_complete"
            and self.mode != "matrix_pow2"
        ):
            # print(cseq)
        ...
        if self.mode == "matrix_slow":
            ...
            fr = torch.gather(
                temp0,
                3,
                self.idx_dec.unsqueeze(0)
                .unsqueeze(0)
                .expand(temp0.shape[0], temp0.shape[1], -1, -1),
            ).sum(2)
            ...
            for j, (fc, gdii_j) in enumerate(zip(cseq, self.gdiis)):
                ...
                for i, (wr1, wr2, Lg) in enumerate(
                    self.loopparams_dec[fbin_ptr : fbin_ptr + nb_fbins][:fbins]
                ):
                    r = (Lg + 1) // 2
                    l = Lg // 2
